...
THRESHOLDS = {'clean': .01, 'execution': .1, 'alignment': .5, 'geometry': .02,'entry':.01}
...
def metrics(report):
    ...
    result = {name: {'clean': s['cleanCompletionRate'], 'execution': s['meanExecution'],
                   'alignment': -s['meanEntryAngle'], 'geometry': s['geometryPassRate'],'entry':s['cleanEntryRate']}
...
def main(args):
    ...
    def assess(checkpoint, directory, label):
        if not directory.exists():
            ...
            run([sys.executable, 'training_v2/freeze_policy.py', str(checkpoint), str(temporary)], label+'-freeze')
            if stopped: return
            os.rename(temporary, directory)
        if stopped: return
        if not all((directory / name).exists() for name in ['evaluation.json', 'cases.json']):
            run([sys.executable, 'training_v2/evaluate.py', '--policy', str(directory/'policy.json'),
                 '--initial', 'training/initial-v8.json', '--per-skill','64','--quick',
                 '--minimum-heights','3','5','7.5','3','7.5','9','--output',str(directory)], label+'-evaluate')
        if stopped: return
        legacy=directory/'legacy-judge'
        if not (legacy/'evaluation.json').exists():
            run([sys.executable,'training/evaluate.py','--policy',str(directory/'policy.json'),'--initial','training/initial-v8.json','--cross-physics-baseline','--per-skill','64','--quick','--minimum-heights','3','5','7.5','3','7.5','9','--output',str(legacy)],label+'-legacy-evaluate')
        if stopped:return
    ...
    try:
        ...
        while not stopped:
            ...
            directory = ROOT/'training_v2'/name
            ...
            if pending and pending['index'] == index and (Path(pending['directory'])/'latest.json').exists():
                ...
                name += '-resume-' + str(time.time_ns()); directory = ROOT/'training_v2'/name
            if directory.exists():
                name += '-retry-' + str(time.time_ns()); directory = ROOT/'training_v2'/name
            remaining = args.chunk_steps - already
            if remaining > 0:
                save(pending={'index':index,'directory':str(directory)})
                run([sys.executable,'training_v2/train.py','--name',name,'--resume',str(checkpoint),
