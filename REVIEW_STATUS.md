...
## v13 final-run configuration (2026-09-15)

The completion-first run (`2026-09-14-bounded-fresh`) reached 100.8M steps with 36% valid dives, 0.27 execution and no clean entry. Open-loop ceiling probes (`review-evidence/2026-09-15/ceiling-probes/`) found 9.05/10 forward dives from 10 m, 8.66 from 5 m and 8.83 from the 3 m springboard within a few hundred random schedules, so the environment is not the limit. Measured causes and the v13 repairs are in `training_v3/FINAL_RUN_PLAN.md` and the README: entry alignment worth 0.36 against 16 for rotation (now a conjunctive credit), an entry sub-task that never succeeded (now started near the entry pose and self-adjusting), full-width stance randomisation from step zero (now a mastery curriculum), 40 ms exploration correlation (now 200 ms with an exact likelihood), no time-to-water input (added), fixed input scaling (running normalisation folded at export), and a clean-form threshold no scripted dive could reach (0.8 to 0.7). The browser runtime and export were moved to the v13 layout (`self-declared-diver-v13`, split trunks, eighteen history columns); native-versus-JavaScript policy parity passes at 3e-9. The run launched as `2026-09-15-v13-final` on the SSD: three seeds of 256x256x128 for 10.24M steps, gates at 10.24M and 30.72M, then the median seed continues. Nothing here is a model-quality claim; the site still serves the 2026-09-11 development export.
