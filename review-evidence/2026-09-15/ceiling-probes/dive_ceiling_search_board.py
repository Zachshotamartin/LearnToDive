...
N = 64
APPARATUS = sys.argv[4] if len(sys.argv) > 4 else 'platform'
...
def run_batch(params, height):
    ...
    p.reset(list(range(N)), [dict(skill=0, height=height, disturbance=0, platform=APPARATUS == 'platform', preload=0 if APPARATUS == 'platform' else -.05123, lean=.04, hip=.12, knee=.2, toeOver=.03)] * N)
    p.platform[:] = APPARATUS == 'platform'; p.headfirst[:] = True; p.goals[:] = [.5, 0, 0, 3]
    ...
    for i, m in enumerate(finished):
        ...
        try:
            s = judge(IDS['101A'], APPARATUS, height, m)
