...
def score(info):
 ...
 entry=info['water'] and info.get('fullEntryComplete',False) and info.get('entryGeometryValid',False) and info.get('entryLimbsValid',False) and angle<35 and form>.55 and info['limbLimits'] and info['geometryLimits'] and info['firstGeometry'] in (6,9) and info['x']>.55
 splash=float(np.clip(.55*np.sin(np.radians(angle))**2+.25*(1-form)+.2*info['horizontalSpeed']/3,0,1))
 ...
 return dict(valid=bool(not info['boardInvalid'] and numeric and position and entry),launchValid=not info['boardInvalid'],numericCorrect=bool(numeric),shapeCorrect=bool(position),validEntry=bool(entry),execution=execution,points=3*execution*DD[sid],difficulty=DD[sid],splash=splash,deductions=deductions)
...
def main(args):
 started=time.time();infer,data=actor(args.policy,allow_cross_physics=args.cross_physics_baseline);initial,initial_data=actor(args.initial)
 ...
 report={'sourcePhysicsSHA256':source_xml_hash,'crossPhysicsBaseline':bool(data.get('crossPhysicsBaseline')),'policyContract':data.get('contract'),'initialContract':initial_data.get('contract'),'crossPhysicsInterpretation':'Explicit cross-physics diagnostic; source and runtime contracts differ. Architectural changes require a separately recorded weight migration.' if data.get('crossPhysicsBaseline') else None,'contract':runtime_contract(),'schemaVersion':2,'seed':args.seed,'modelSteps':data['steps'],'minimumHeights':args.minimum_heights or [s['height']for s in SKILLS],'policySHA256':data['loadedSHA256'],'initialSHA256':initial_data['loadedSHA256'],'physicsSHA256':hashlib.sha256((ROOT/'public/physics/diver.xml').read_bytes()).hexdigest(),'judge':JUDGE_VERSION,'sourceSHA256':{name:hashlib.sha256((ROOT/'training'/name).read_bytes()).hexdigest() for name in ['sim.py','evaluate.py']},'policies':{},'perSkill':{},'heightGrid':{},'outOfDistribution':{},'feedback':{}}
 all_cases={}
