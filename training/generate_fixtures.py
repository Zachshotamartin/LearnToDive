...
import json,hashlib,argparse,mujoco,numpy as np
from pathlib import Path
from contract import runtime_contract
from sim import Arena,ROOT
from evaluate import baseline,score
parser=argparse.ArgumentParser();parser.add_argument('--output',type=Path,default=ROOT/'tests/fixtures/mujoco-parity.json');args=parser.parse_args()
cases=[]
...
sources={name:hashlib.sha256(Path(__file__).with_name(name).read_bytes()).hexdigest() for name in ['sim.py','water.py','evaluate.py','rewards.py','contract.py','generate_fixtures.py']}
args.output.parent.mkdir(parents=True,exist_ok=True)
args.output.write_text(json.dumps({'engine':'MuJoCo'+mujoco.__version__,'contract':runtime_contract(),'sources':sources,'cases':cases},separators=(',',':')))
print('Wrote independent native fixtures:',args.output)
