...
class Arena:
 def __init__(self,n=128,seed=123,threads=8,skills=None,heights=(3,10),training=True,minimum_heights=None,boundary_rate=0,execution_weight=0,ascent_weight=8,launch_speed_weight=.5,entry_geometry_weight=0):
 ...
 def track_entry_batch(self,ids,state,sensor):
  ...
  geometry=entry_geometry(sensor);form,limbs,limits,_=entry_posture(state[:,1+self.qadr],geometry)
  ...
  self.entry_worst_geometry[ids]=np.maximum(self.entry_worst_geometry[ids],metrics);self.entry_crossed[ids]|=geometry['crossedLegs']
  self.entry_max_angle[ids]=np.maximum(self.entry_max_angle[ids],angle);self.entry_min_form[ids]=np.minimum(self.entry_min_form[ids],form)
  self.entry_limbs_valid[ids]&=limbs;self.entry_geometry_valid[ids]&=limits;self.entry_samples[ids]+=1
 ...
 def step(self,action,auto_reset=True):
  ...
  entry_angle=np.maximum(first_angle,self.entry_max_angle);first_form=straight.copy();straight=np.minimum(straight,self.entry_min_form)
  splash=np.clip(.55*np.sin(np.radians(entry_angle))**2+.25*(1-straight)+.2*np.linalg.norm(s[:,3:5],axis=1)/3,0,1)
  numeric=(np.abs(err)<.45)&(np.abs(twerr)<.55)&(self.max_lateral<.85)&contacted
  position=(g[:,3]==2)|(shape_fraction>=.65)
  entry=contacted&np.isfinite(self.full_entry_time)&(entry_angle<35)&(straight>.55)&limb_limits&geometry_limits&self.entry_limbs_valid&self.entry_geometry_valid&np.isin(self.first_geometry,[6,9])&(judging_s[:,0]>.55)
  ...
  terminal=8*rotation*twisting*clearance
  terminal+=24*rotation*twisting*terms['quality']*clearance*position*contacted
