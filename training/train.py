...
def episode_statistics(episode,skills):
 ...
 groups={}
 fields={'rotation':'rotation','valid':'valid','postureCoverage':'shapeFraction','meanAscent':'ascent','meanTakeoffVerticalSpeed':'takeoffVerticalSpeed','meanEntryAngle':'entryAngle','meanPreparationBounces':'preparationBounces','boardPeak':'boardNonfootPeak'}
 for skill in skills:
  rows=[r for r in episode if r['skill']==skill]
 ...
 return {'return':mean(episode,'return_'),'valid':mean(episode,'valid'),'skills':groups}
...
def train(args):
 ...
 np.random.seed(args.seed);torch.manual_seed(args.seed);env=Arena(args.envs,seed=args.seed,threads=args.threads,skills=args.skills,heights=(args.min_height,args.max_height),minimum_heights=args.skill_min_heights,boundary_rate=args.boundary_rate,execution_weight=args.execution_weight,ascent_weight=args.ascent_weight,launch_speed_weight=args.launch_speed_weight,entry_geometry_weight=args.entry_geometry_weight)
 ...
 for path in [ROOT/'training/sim.py',ROOT/'training/train.py',ROOT/'training/contract.py',ROOT/'training/evaluate.py',ROOT/'src/core/control.js',ROOT/'src/core/physics.js',ROOT/'public/physics/diver.xml',ROOT/'training/water.py',ROOT/'src/core/water.js',ROOT/'training/critic_calibration.py',ROOT/'training/checkpoints.py',ROOT/'training/rewards.py']:
 ...
 contract=runtime_contract(source/'diver.xml');contract['trainingSourceSHA256']=json.loads((source/'sha256.json').read_text());contract['stepsOnContract']=0;contract['actorUpdateInteractions']=0;contract['criticCalibrationInteractions']=0;contract['criticCalibrationRetainedSamples']=0
 ...
 if args.resume:
  ...
  try:
   ...
   provenance['physicsMigration']={'parentContract':parent_contract,'parentXMLSHA256':c.get('provenance',{}).get('sourceHashes',{}).get('diver.xml'),'description':('V7 to v8 full-entry extension:72 existing observations and9 actions retained, four zero input columns appended, fresh optimizer.' if c['model']['actor.0.weight'].shape[1]==72 else 'Explicit same-architecture physical-contract migration: compare parent/current XML hashes. Head sphere matched to the existing shared figure radius (0.094m), keeping5kg mass, and full-depth basin side walls repaired; motor/control ranges and water law unchanged. Fresh optimizer and zero current-contract decisions.')}
...
def parse_args(argv=None):
 ...
 p.add_argument('--resume-mode',choices=['exact','restart'],default='exact');p.add_argument('--threads',type=int,default=8);p.add_argument('--checkpoint-every-steps',type=int,default=204800);p.add_argument('--checkpoint-every-seconds',type=float,default=300)
